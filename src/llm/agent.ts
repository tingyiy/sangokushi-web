...
/**
 * Sync new war log entries from the game store into the LLM's permanent warRecords.
 * Called after endTurn and after battles so the LLM remembers ALL wars.
 */
function syncWarLog(): void {
  const warLog = useGameStore.getState().warLog;
  const newEntries = warLog.slice(_lastSeenWarLogIndex);
  _lastSeenWarLogIndex = warLog.length;
  const pf = useGameStore.getState().playerFaction;
  for (const w of newEntries) {
    const weWereAttacked = w.defenderFactionId === pf?.id;
    const weAttacked = w.attackerFactionId === pf?.id;
    const result = w.attackerWon
      ? `${w.attackerFaction} captured ${w.city}`
      : `${w.defenderFaction} repelled the attack`;
    recordWar({
      year: w.year,
      month: w.month,
      aggressorFaction: w.attackerFaction,
      defenderFaction: w.defenderFaction,
      city: w.city,
      aggressorWon: w.attackerWon,
      weAttacked,
      weWereAttacked,
      summary: `${w.type}. ${result}.${weAttacked || weWereAttacked ? '' : ' (other factions)'}`,
    });
  }
}
...
export async function runStrategicTurn(): Promise<boolean> {
  ...
  for (let actionNum = 0; actionNum < MAX_ACTIONS_PER_TURN; actionNum++) {
    ...
    if (cmdType === 'endTurn') {
      ...
      // Sync any new war log entries from AI battles during endTurn
      syncWarLog();
...
async function runBattlePhase(): Promise<void> {
  ...
  recordBattleStart({
    type: battle.isSiege ? 'siege' : 'field',
    ourOfficers: playerUnits.map(u => u.officer.name),
    enemyOfficers: enemyUnits.map(u => u.officer.name),
    ourTroops: playerTroops,
  ...
  while (maxIterations-- > 0) {
    ...
    if (bState.isFinished || gState.phase !== 'battle') {
      ...
      // Collect rich battle end data
      const endPlayerUnits = bState.units.filter(u => u.factionId === bState.playerFactionId);
      const endEnemyUnits = bState.units.filter(u => u.factionId !== bState.playerFactionId);
      const endPlayerTroops = endPlayerUnits.reduce((s, u) => s + u.troops, 0);
      const endEnemyTroops = endEnemyUnits.reduce((s, u) => s + u.troops, 0);

      // Determine outcome reason
      let outcomeReason = 'unknown';
      const loserFId = outcome === 'VICTORY'
        ? (bState.playerFactionId === bState.attackerId ? bState.defenderId : bState.attackerId)
        : bState.playerFactionId;
      const loserUnits = bState.units.filter(u => u.factionId === loserFId);
      // Commander = first unit of the faction (RTK IV rule)
      const loserCommander = loserUnits[0];
      if (loserCommander && loserCommander.troops <= 0) {
        outcomeReason = 'Commander defeated';
      } else if (loserUnits.every(u => u.troops <= 0 || u.status === 'routed')) {
        outcomeReason = 'All units eliminated/routed';
      } else if (bState.day >= 30) {
        outcomeReason = 'Day limit (30) reached — attacker loses';
      }

      endBattleMemory(outcome, {
        days: bState.day,
        outcomeReason,
        ourTroopsEnd: endPlayerTroops,
        enemyTroopsEnd: endEnemyTroops,
        officersCaptured: bState.capturedOfficerIds
          .map(id => bState.units.find(u => u.officerId === id)?.officer.name)
          .filter((n): n is string => !!n),
        officersLost: endPlayerUnits
          .filter(u => u.troops <= 0 || u.status === 'routed')
          .map(u => u.officer.name),
      });
      ...
      if (gState.phase === 'battle' && bState.isFinished && winner !== null) {
        ...
        syncWarLog(); // Capture war record from resolveBattle
